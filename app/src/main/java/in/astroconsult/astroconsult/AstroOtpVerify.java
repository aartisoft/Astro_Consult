...
public class AstroOtpVerify extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_astro_otp_verify);
        pinEntry = (PinEntryEditText) findViewById(R.id.Astro_txt_pin_entry);
        progressBar = findViewById(R.id.astro_progress_bar);
        verify = findViewById(R.id.AstrootpButton);
        resendOtp = findViewById(R.id.AstroresendOtp);

        mAuth = FirebaseAuth.getInstance();
        mDatabaseRef = FirebaseDatabase.getInstance().getReference().child("Users");

        phone = getIntent().getStringExtra("mobileAstro");
    ...
    private PhoneAuthProvider.OnVerificationStateChangedCallbacks mCallbacks = new PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
        ...
        public void onVerificationFailed(@NonNull FirebaseException e) {
            Toast.makeText(AstroOtpVerify.this, ""+e.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    private void signInTheUserByCredentials(PhoneAuthCredential credential) {
        ...
        firebaseAuth.signInWithCredential(credential)
                .addOnCompleteListener(AstroOtpVerify.this, new OnCompleteListener<AuthResult>() {
                    @Override
                    public void onComplete(@NonNull Task<AuthResult> task) {

                        if (task.isSuccessful()) {

                            AstroLogInPreference.getInstance(AstroOtpVerify.this).setLogged(true);
                            FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
                            String currentUserId = currentUser.getUid();
                            FirebaseAuthUtil.getInstance(mAuth,mDatabaseRef).getAstro(getApplicationContext(),currentUserId);

                            Toast.makeText(AstroOtpVerify.this, "Otp Verified",Toast.LENGTH_SHORT).show();
                            Intent intent = new Intent(getApplicationContext(), AstroHome.class);
                            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                            startActivity(intent);
                        } else {
                            Toast.makeText(AstroOtpVerify.this, ""+task.getException().getMessage(), Toast.LENGTH_LONG).show();
                        }
                    }
                });
    }
